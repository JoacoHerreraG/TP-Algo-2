...
/** -----------------------NUESTROS------------------------------------
**/
TEST(test_bd, test_proj_idempotente){
    Driver d;
    d.leerDataset("trenes");
    ASSERT_EQ(d.consultar(Consulta("from(trenes)")),d.consultar(Consulta(
        "proj(from(trenes), {id,mes,linea,estacion,cantidad})"
        ))
    );
}

TEST(test_bd, test_doble_rename){
    Driver d;
    d.leerDataset("trenes");
    ASSERT_EQ(d.consultar(Consulta(
        "rename(rename(from(trenes),linea,Linea),Linea,Linea)")),
    d.consultar(Consulta(
               "rename(from(trenes),linea,Linea)"))
    );
}

TEST(test_bd, test_rename_idempotente){
    Driver d;
    d.leerDataset("trenes");
    ASSERT_EQ(d.consultar(Consulta(
        "rename(rename(from(trenes),linea,Linea),Linea,linea)")),
...
TEST(test_bd, test_renameMasselect){

    Driver d;
    d.leerDataset("trenes");
    Respuesta res1 = d.consultar(Consulta(
        "select("
        "select("
        "rename(rename(from(trenes),mes, Mes),linea,Linea),"
        "Mes,'12/2018'"
        "),"
        "Linea, 'belgranosur'"
        ")"));
    Respuesta res1bis = d.consultar(Consulta(
        "select(select(from(trenes),mes,'12/2018'),linea,'belgranosur')"));
    ASSERT_EQ(res1.size(), res1bis.size());
    //Respuesta res2 = d.consultar(Consulta(
    //  "rename(rename(from(trenes),estacion,Estacion),cantidad,Cantidad)"));

}

/* incompleto
TEST(test_bd, test_rename+select+proy+inter_){
    Driver d;
    d.leerDataset("trenes");
    Respuesta res1 = d.consultar(Consulta(
        "proy(select(select(rename(rename(from(trenes),mes,Mes),linea,Linea),Mes,12/2018),Linea,belgranosur),
        {estacion,cantidad})"));
    Respuesta res1bis = d.consultar(Consulta(
       "proy(select(select(rename(rename(from(trenes),estacion,Estacion),cantidad,Cantidad),mes,12/2018),
       linea,belgranosur)
       ,{Estacion,Cantidad})"))
    Respuesta res2 = d.consultar(Consulta(
        "rename(rename(from(trenes),estacion,Estacion),cantidad,Cantidad)"));
...
TEST(test_bd, borrar_tablas) {
    Driver a;
    a.leerDataset("select_estacion_constitucion");
    a.leerDataset("select_linea_roca");
    a.leerDataset("select_linea_roca_temperley");
    a.leerDataset("union_temperley_sanIsidro");
    a.leerDataset("proj_select_estacion_cantidad");
    ASSERT_EQ(a.getCantTabla(), 5);

    a.borrarTabla("select_estacion_constitucion");
    a.borrarTabla("select_linea_roca");
    ASSERT_EQ(a.getCantTabla(), 3);



    a.borrarTabla("select_linea_roca_temperley");
    a.borrarTabla("union_temperley_sanIsidro");
    a.borrarTabla("proj_select_estacion_cantidad");
    ASSERT_EQ(a.getCantTabla(), 0);


}

TEST(test_bd, borrar_registros) {
    Driver d;
    d.leerDataset("trenes");
    list<Registro> r = d.darTabla("trenes").registros();
    for (auto it = r.begin() ; it != r.end() ; it++){
        d.eliminarRegistro("trenes", *it);

    }
    r = d.darTabla("trenes").registros();
    ASSERT_EQ(r.size(), 0);

}


TEST(test_bd, doble_borrado_registro) {
    Driver d;
    d.leerDataset("trenes");
    list<Registro> r = d.darTabla("trenes").registros();
    auto it = r.begin();
